...
class StockModel {
   ...
   */
  static async registerMovement({ product_id, barcode, movement_type, quantity, notes = '' }) {
    const lookupKey = product_id || barcode;
    const product = await ProductModel.getById(lookupKey);
    if (!product) {
      throw new Error(`Producto con código/SKU "${lookupKey}" no encontrado.`);
    }
    ...
    if (['SALIDA_VENTA', 'MERMA_PERDIDA', 'SALIDA'].includes(movement_type)) {
      delta = -qty;
    }
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        // 1. Actualizar el stock del producto usando sku como PK
        const updateStockSql = `UPDATE products SET stock = ?, updated_at = CURRENT_TIMESTAMP WHERE sku = ?`;
        db.run(updateStockSql, [newStock, skuKey], function (err) {
          if (err) {
            db.run('ROLLBACK', () => {}); // callback vacío para evitar crash si no hay tx activa
            return reject(err);
          }
          ...
          const insertMovementSql = `
            ...
            VALUES (?, ?, ?, ?, ?, ?)
          `;
          db.run(insertMovementSql, [skuKey, movement_type, qty, previousStock, newStock, notes], function (err2) {
            if (err2) {
              db.run('ROLLBACK', () => {}); // callback vacío para evitar crash si no hay tx activa
              return reject(err2);
            }
  ...
  static async getMovements({ product_id = null, limit = 50 } = {}) {
    let sql = `
      SELECT m.*, p.name as product_name, p.sku, p.barcode, p.unit_measure
      FROM stock_movements m
      JOIN products p ON m.product_id = p.sku
      WHERE 1=1
    `;
    const params = [];

    if (product_id) {
      sql += ` AND (m.product_id = ? OR p.barcode = ?)`;
