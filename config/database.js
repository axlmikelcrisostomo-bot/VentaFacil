...
const dbPath = path.join(__dirname, '..', 'database', 'ventafacil.db');
...
// Crea las tablas una a una usando db.run() para evitar problemas de parsing de sqlite3 con exec()
function initDatabase() {
  ...
  return new Promise((resolve, reject) => {
    // Ejecutar cada statement secuencialmente con callback recursivo
    function runNext(index) {
      if (index >= statements.length) {
        console.log('Esquema de base de datos e índices inicializados correctamente.');
        resolve();
        return;
      }
      db.run(statements[index], (err) => {
        if (err) {
          console.error(`Error en sentencia ${index}:`, err.message);
          reject(err);
        } else {
          runNext(index + 1);
        }
...
// Wrapper para ejecutar consultas tipo Async/Await (Promisify)
